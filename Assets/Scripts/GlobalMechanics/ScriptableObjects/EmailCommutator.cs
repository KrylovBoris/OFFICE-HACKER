...
namespace GlobalMechanics.ScriptableObjects
{
    ...
    {
        public string emailDomain = "companyname.com";
        ...
        public void EstablishConnection(string newClientAddress, out EmailBox emailBox)
        {
            EmailBox newClient;
            if (!newClientAddress.Contains("@"))
            {
                newClient = new EmailBox(newClientAddress + '@' + emailDomain);
            }
            else
            {
                newClient = new EmailBox(newClientAddress);
            }
